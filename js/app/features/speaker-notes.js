...
export function getStableNoteKey(slide) {
    const explicitKey = slide.dataset.noteKey?.trim();
    return explicitKey || slide.id.trim() || null;
}
export function getCuratedNoteBlock(locale, key) {
    if (locale === 'pt-BR')
        return null;
    return CURATED_NOTES_BY_ID[locale]?.[key] ?? null;
}
export function findDuplicateNoteKeys(keys) {
    const seen = new Set();
...
export function resolveNoteSource(locale, key, hasDirectNote) {
    if (hasDirectNote)
        return 'direct';
    if (key && getCuratedNoteBlock(locale, key))
        return 'curated';
    if (key)
        return 'missing-essential';
    return 'fallback';
}
function appendValidationError(slide, message) {
    if (slide.querySelector(':scope > [data-note-validation-error]'))
        return;
...
export function ensureSpeakerNotesParity(locale, validationMode = false) {
    ...
    slides.forEach((slide, index) => {
        ...
        if (source === 'missing-essential' && key) {
            ...
            console.error(message);
            if (validationMode)
                appendValidationError(slide, message);
            return;
        }
        if (locale === 'pt-BR')
            return;
