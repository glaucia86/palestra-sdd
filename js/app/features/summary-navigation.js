...
export function bindSummaryNavigation() {
    const summary = document.getElementById('sumario');
    if (!summary || summary.dataset.navigationBound === 'true')
        return;
    ...
    summary.addEventListener('click', (event) => {
        ...
        if (!(origin instanceof Element))
            return;
        const link = origin.closest(SUMMARY_LINK_SELECTOR);
        if (!link)
            return;
...
//# sourceMappingURL=summary-navigation.js.map