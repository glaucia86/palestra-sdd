...
let hyperdriveTapTimes = [];
let endAudioButton = null;
let endAudioTextEl = null;
let endAudioHintEl = null;
const THE_END_MESSAGES = {
    'pt-BR': {
...
function updateTheEndUi(slide) {
    ...
    if (endAudioButton)
        endAudioButton.setAttribute('aria-pressed', String(endAudioEnabled));
    if (endAudioTextEl)
        endAudioTextEl.textContent = endAudioEnabled ? copy.buttonOn : copy.buttonOff;
    if (endAudioHintEl) {
...
function clearHyperdriveTapWindow() {
    hyperdriveTapTimes = [];
...
function setHyperdriveMode(slide, enabled) {
    ...
    if (finalSongEl)
        finalSongEl.playbackRate = enabled ? 1.035 : 1;
    updateTheEndUi(slide);
}
function registerHyperdriveTap(slide) {
    const now = Date.now();
    ...
    if (hyperdriveTapTimes.length < 3)
        return false;
    clearHyperdriveTapWindow();
    setHyperdriveMode(slide, true);
    return true;
}
async function toggleTheEndSong(slide, forceState = null, resetOnStop = false) {
...
export function syncTheEndExperience(currentSlide) {
    const theEndSlide = document.getElementById(THE_END_SLIDE_ID);
    if (!theEndSlide)
        return;
    if (!endAudioBound) {
        const toggleButton = theEndSlide.querySelector('[data-end-audio-toggle]');
        const textEl = theEndSlide.querySelector('[data-end-audio-text]');
        const hintEl = theEndSlide.querySelector('[data-end-audio-hint]');
...
//# sourceMappingURL=the-end-experience.js.map