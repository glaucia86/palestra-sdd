...
const CURATED_TALK_TRACK: Record<Exclude<Locale, 'pt-BR'>, NoteBlock[]> = {
  'en-US': [
...
function collectKeyPoints(section: HTMLElement): string[] {
  ...
  const candidates = Array.from(section.querySelectorAll<HTMLElement>(selectors))
    .map((el: HTMLElement) => normalizeText(el.textContent ?? ''))
    .filter((text: string) => text.length >= 24 && text.length <= 120);
  const unique: string[] = [];
  candidates.forEach((point: string) => {
...
export function ensureSpeakerNotesParity(locale: Locale): void {
  if (locale === 'pt-BR') return;

  const curated = CURATED_TALK_TRACK[locale];
  const fallback = FALLBACK_BY_LOCALE[locale];
  if (!curated || !fallback) return;
  ...
  slides.forEach((slide: HTMLElement, index: number) => {
    ...
    trackIndex += 1;
    if (block?.length) {
      block.forEach((line: string) => appendParagraph(note, line));
    } else {
