...
const VALIDATION_MESSAGES: Record<Locale, QuizValidationMessages> = {
  'pt-BR': {
    ...
    invalidOptionsCount: (qNum: number): string => `Questão ${qNum}: "options" deve ter exatamente 4 itens.`,
    invalidOptionsValue: (qNum: number): string => `Questão ${qNum}: todas as "options" devem ser textos não vazios.`,
  ...
  'en-US': {
    ...
    invalidOptionsCount: (qNum: number): string => `Question ${qNum}: "options" must contain exactly 4 entries.`,
    invalidOptionsValue: (qNum: number): string => `Question ${qNum}: every "option" must be non-empty text.`,
  ...
  'es-ES': {
    ...
    invalidOptionsCount: (qNum: number): string => `Pregunta ${qNum}: "options" debe contener exactamente 4 elementos.`,
    invalidOptionsValue: (qNum: number): string => `Pregunta ${qNum}: todas las "options" deben ser textos no vacíos.`,
